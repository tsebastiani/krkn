...
function functional_test_telemetry {
  ...
  $AWS s3 ls "s3://$AWS_BUCKET/$RUN_FOLDER/" | awk '{ print $4 }' > remote_files
  cat remote_files | grep events-00.json || ( echo "FAILED: events-00.json not uploaded" && exit 1 )
  cat remote_files | grep prometheus-00.tar || ( echo "FAILED: prometheus backup not uploaded" && exit 1 )
  cat remote_files | grep telemetry.json || ( echo "FAILED: telemetry.json not uploaded" && exit 1 )
